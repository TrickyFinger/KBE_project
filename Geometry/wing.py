...
from Geometry.aileron import Movable
...
class Wing(GeomBase):
    ...
    def wing_area_inboard(self):
        return 0.5 * (self.wing_c_root + self.wing_c_kink) * 2 * self.kink_span
    ...
    def wing_aspect_ratio(self):
        return (2 * self.span_wing) / \
               (self.wing_c_root * (((1 - self.full_wing_taper_ratio) * (2 * self.kink_span / self.span_wing)) +
                                       (self.inboard_taper + self.full_wing_taper_ratio)))

    # calculates the length of the wing tip chord
    @Attribute
    def wing_c_tip(self):
        return self.full_wing_taper_ratio * self.wing_c_root

    # calculates the length of the kink
    @Attribute
    def wing_c_kink(self):
        return self.wing_c_root * self.inboard_taper
    ...
    def y_mac_in(self):
        return (self.span_wing / 15) * (
                (self.wing_c_root + (2 * self.wing_c_kink)) / (self.wing_c_root + self.wing_c_kink))
    ...
    def starting_point_mac(self):
        x_start_mac = (self.y_mac * tan(radians(self.wing_sweep_025c)) -
                       (0.25 * self.MAC_chord_length) + 0.25 * self.wing_c_root)  # + (0.48 * self.cabin_l)
        return x_start_mac
    ...
    @Attribute
    def chords(self):
        return [self.wing_c_root, self.wing_c_kink, self.wing_c_tip]
    ...
    @Part
    def movable_element_right(self):
        return Movable(loft=self.solid,
                       rootcrv=self.sections[0],
                       tipcrv=self.sections[1],
