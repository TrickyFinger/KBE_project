...
class ReadGeometry:
    def __init__(self, name):
        ...
        for i, line in enumerate(searchlines):
            if "#Fuselage" == line.rstrip():
                dat_fuselage = np.array(searchlines[i+1].split(), dtype=float)
                self.ln_d = dat_fuselage[0]
                self.lt_d = dat_fuselage[1]
                self.cabin_d = dat_fuselage[2]
            ...
            if "#HorizontalTail" == line.rstrip():
                ht_airfoils = searchlines[i+1].split()
                self.ht_root = ht_airfoils[0]
                self.ht_tip = ht_airfoils[1]

                dat_ht = np.array(searchlines[i+2].split(), dtype=float)
                self.htp_area = dat_ht[0]
                self.htp_taper = dat_ht[1]
                self.htp_dihedral = dat_ht[2]

            if "#VerticalTail" == line.rstrip():
                vt_airfoils = searchlines[i+1].split()
                self.vt_root = vt_airfoils[0]
                self.vt_tip = vt_airfoils[1]

                dat_vt =np.array(searchlines[i+2].split(), dtype=float)
                self.vtp_aspect_ratio = dat_vt[0]
                self.vtp_sweep = dat_vt[1]
                self.vtp_area = dat_vt[2]
